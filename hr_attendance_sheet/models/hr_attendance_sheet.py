...
class HrAttendanceSheet(models.Model):
    ...
    hours_to_work = fields.Float(
        related="employee_id.hours_to_work", string="Hours to Work",
    ...
    can_review = fields.Boolean(string="Can Review",
        ...
        for sheet in self:
            if sheet.state == 'draft':
                to_clean |= sheet
                    ...
                    (sheet.review_policy == 'employee_manager' or
                     sheet.review_policy == 'hr_or_manager'):
                if sheet.sudo().employee_id.parent_id.user_id.id:
                    sheet.activity_schedule(
                        'hr_attendance_sheet.'
                        'mail_act_attendance_sheet_approval',
    ...
    def _check_can_review(self):
        if self.filtered(lambda x: not x.can_review and
                         x.review_policy == 'hr'):
            raise UserError(_("""Only a HR Officer can review the sheet."""))
        if self.filtered(lambda x: not x.can_review and
                         x.review_policy == 'employee_manager'):
            raise UserError(_("""Only the Manager of the Employee can review
            the sheet."""))
        if self.filtered(lambda x: not x.can_review and
                         x.review_policy == 'hr_or_manager'):
            raise UserError(_("""Only the Manager of the Employee or an HR
    ...
    def write(self, values):
        ...
        if self.state == 'locked' and \
                any(f in values.keys() for f in protected_fields):
            raise UserError(_("You can't edit a locked sheet."))
        elif self.state in ('confirm', 'done') and self.env.user not in \
                self._get_possible_reviewers():
            raise UserError(_(
    ...
    def action_attendance_sheet_done(self):
        ...
        for sheet in self:
            ...
            if not ids_not_checkout:
                ...
                if not reviewer:
                    raise UserError(_("""In order to review a attendance sheet,
    ...
    def action_attendance_sheet_lock(self):
        ...
        elif self.env.user.has_group('hr_attendance.group_attendance_user'):
    ...
    def action_attendance_sheet_unlock(self):
        ...
        if self.env.user.has_group('hr_attendance.group_attendance_user'):
